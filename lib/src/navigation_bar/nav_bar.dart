...
class DesktopNavBar extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isScrolled = ref.watch(scrolledProvider).state;
      ...
      child: Padding(
        ...
        child: Row(
          children: <Widget>[
            ...
            NavBarButton(
              onTap: () => ref.read(currentPageProvider).state = homeKey,
              text: "Home",
            ),
            NavBarButton(
              onTap: () => ref.read(currentPageProvider).state = featureKey,
              text: "Features",
            ),
            NavBarButton(
              onTap: () => ref.read(currentPageProvider).state = screenshotKey,
              text: "Screenshots",
            ),
            NavBarButton(
              onTap: () => ref.read(currentPageProvider).state = contactKey,
              text: "Contact",
            ),
          ],
        ),
            ...
            child: Column(
              children: [
                NavBarButton(
                  text: "Home",
                  onTap: () {
                    ref.read(currentPageProvider).state = homeKey;
                ...
                NavBarButton(
                  text: "Features",
                  onTap: () {
                    ref.read(currentPageProvider).state = featureKey;
                    containerHeight.value = 0;
                  },
                ),
                NavBarButton(
                  text: "Screenshots",
                  onTap: () {
                    ref.read(currentPageProvider).state = screenshotKey;
                    containerHeight.value = 0;
                  },
                ),
                NavBarButton(
                  text: "Contact",
                  onTap: () {
                    ref.read(currentPageProvider).state = contactKey;
                    containerHeight.value = 0;
                  },
                ),
              ],
